...
class medical_file_model extends CK_Model{
	...
	public function insertNewMedicalFile($summerCampId, $colonistId, $bloodType, $rh, $weight, $height, $physicalActivityRestriction,
		...
		$medicineRestrictions, $allergies, $analgesicAntipyretic, $doctorId, $specialCareMedical, $psychMedication){
		...
		medicine_restrictions, allergies, analgesic_antipyretic, doctor_id, special_care, psych_medication) VALUES (?, ?, ?, ?, ?,?, ?, ?, ?,?, ?, ?, ?,?, ?, ?, ?, ?, ?)';
		...
		$vacineTetanus, $vacineMMR, $vacineHepatitis, $vacineYellowFever, $infectoContagiousAntecedents, $regularUseMedicine, 
		$medicineRestrictions, $allergies, $analgesicAntipyretic, intval($doctorId), $specialCareMedical, $psychMedication));
		if($returnId)
			return $returnId;
	...
	public function updateMedicalFile($summerCampId, $colonistId, $bloodType, $rh, $weight, $height, $physicalActivityRestriction,
		...
		vacine_tetanus = ?, vacine_mmr = ?, vacine_hepatitis = ?, vacine_yellow_fever = ?, infecto_contagious_antecedents = ?, regular_use_medicine = ?, 
		medicine_restrictions = ?, allergies = ?, analgesic_antipyretic = ?, doctor_id =?, special_care = ?, psych_medication = ? where summer_camp_id = ? and colonist_id = ?';
		$result = $this->execute($this->db, $sql, array(intval($bloodType), $rh, $weight, $height, $physicalActivityRestriction,
		$vacineTetanus, $vacineMMR, $vacineHepatitis, $vacineYellowFever, $infectoContagiousAntecedents, $regularUseMedicine, 
		$medicineRestrictions, $allergies, $analgesicAntipyretic, intval($doctorId), $specialCareMedical, $psychMedication, intval($summerCampId), intval($colonistId)));
	...
	public function updateStaffMedicalFile($personId, $bloodType, $rh, $weight, $height, $physicalActivityRestriction,
		...
		vacine_tetanus = ?, vacine_mmr = ?, vacine_hepatitis = ?, vacine_yellow_fever = ?,  infecto_contagious_antecedents = ?, regular_use_medicine = ?, 
		medicine_restrictions = ?, allergies = ?, analgesic_antipyretic = ?, doctor_id =?, special_care = ? , psych_medication = ? where person_id = ?';
		$result = $this->execute($this->db, $sql, array(intval($bloodType), $rh, $weight, $height, $physicalActivityRestriction,
		$vacineTetanus, $vacineMMR, $vacineHepatitis, $vacineYellowFever, $infectoContagiousAntecedents, $regularUseMedicine, 
		$medicineRestrictions, $allergies, $analgesicAntipyretic, intval($doctorId), $specialCareMedical, $psychMedication, intval($personId)));
