...
class cielotransaction_model extends CK_Model {
    ...
    public function getPaymentsDetailed($ano = FALSE, $mes = FALSE) {
        $this->Logger->info("Running: " . __METHOD__);
        $where = "";
        if ($ano) {
            $where = " where to_char(ct.date_created, 'YYYY')::integer = " . $ano . "";
            if ($mes) {
                $where .= " and to_char(ct.date_created, 'MM')::integer = " . $mes . "";
            }
        } else {
            if ($mes) {
                $where = " where to_char(ct.date_created, 'MM') = " . $mes . "";
            }
        }
