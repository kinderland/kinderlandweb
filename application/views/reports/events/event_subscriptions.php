<html lang="pt-br">
    ...
    function gerarTXTcomTelefones() {
        ...
        function getTXTName(type) {
            ...
            var filtroNomeConvite = filtros[1].value;
            var e = document.getElementById("colonia");
            var filtroSexo = filtros[2].value;
            var filtroNomeResponsavel = filtros[0].value;
            var filtroIdade = filtros[3].value;
            var filtroPernoite = filtros[4].value;
            var nomePadrao = "sms";

            if (filtroNomeConvite == "" && filtroNomeResponsavel == "") {

                if (filtroSexo == -1) {

                    nomePadrao = nomePadrao.concat("_todos_sexos");
                }

                else {
                    nomePadrao = nomePadrao.concat("_".concat(filtroSexo));
                }

                if (filtroIdade == -1) {

                    nomePadrao = nomePadrao.concat("_todas_idades");
                }

                else {
                    nomePadrao = nomePadrao.concat("_".concat(filtroIdade));
                }

                if (filtroPernoite == -1) {

                    nomePadrao = nomePadrao.concat("_com_e_sem_pernoite");
                }

                else {

                    if(filtroPernoite == "Não")
                    	nomePadrao = nomePadrao.concat("_".concat("_sem_pernoite"));
                    else if(filtroPernoite == "Sim")
                    	nomePadrao = nomePadrao.concat("_".concat("_com_pernoite"));
                }
                ...
                if (filtroNomeResponsavel == "") {
                    return nomePadrao.concat("_filtrado_por_convite_".concat(filtroNomeConvite));
                }
                else if (filtroNomeConvite == "") {
                    ...
                    return nomePadrao.concat("_e_por_convite_".concat(filtroNomeConvite));
                }
            }   			
        }
	...
	function showCounter(currentPage, totalPage, firstRow, lastRow, totalRow, totalRowUnfiltered) {
        ...
        $(document).ready(function() {
			$('#sortable-table').datatable({
				...
				sort : [true,sortLowerCaseLink, sortLowerCase,true,true,true],
				filters : [false,true,true, selectTodos, selectTodas,selectTodos],
		...
		<div class="main-container-report">
					...
					style="max-width: 900px;" id="sortable-table">
							<tr>
							...
							foreach($info as $i){
								...
								<tr>
									...
									<td><a name="responsavel" key="<?= $i->person_user_id; ?>" target="_blank" href="<?= $this->config->item('url_link') ?>user/details?id=<?= $i->person_user_id ?>"><?= $i->responsable_name ?></a></td>
									<td><?php echo $i->fullname;?></td>
									<td><?php if($i->gender === 'M') echo 'Masculino'; else echo "Feminino";?></td>
